...
A **recognition + device-fragment** live-attach mode switch (R44) replaces the frozen full-domain-XML toggle so a hotplug on/off **never** wipes unrelated VM edits — the toggle edits the live XML in place, adding/removing only the GPU (+ audio) `hostdev`, so RAM/CPU/disk, Looking Glass, and stealth/perf tuning survive every toggle (with a device-diff safety invariant that refuses to define if anything else changed). See [System-tray hotplug on/off toggle](#system-tray-hotplug-onoff-toggle-r41).
...
**Recognition + device fragments (R44) — unrelated VM edits survive every toggle.** The R41/R42 frozen-full-domain-XML switch above is **superseded** in vfio 7.3: a toggle no longer `virsh define`s a saved with-GPU/without-GPU dump taken at install time (which would wipe any edit made afterward in virt-manager — RAM/CPU/disk, Looking Glass `<shmem>`, stealth/perf tuning). Instead the **CURRENT libvirt domain XML** (`virsh dumpxml`) is the single source of truth, and the only on-disk state is small **device fragments** (`profiles/<dom>/devices/{gpu,gpu-audio}.xml`) + a derived manifest. A mode switch **edits the live XML in place** — adding/removing ONLY the GPU (+ audio) `hostdev` — so every unrelated device survives unchanged:
- `profile_recognize` reads the live XML and classifies the mode (has the GPU `hostdev` → `off`; no GPU but a saved fragment → `on`; neither → `unknown`), writes/updates a per-domain manifest, and reconciles the mode file (live XML wins; on a disagreement it prints a `MISMATCH` and rewrites the file).
- `profile_apply_mode` does the in-place edit: `on` extracts the GPU (+ first audio) fragment from the CURRENT XML (overwriting so `rom file=`/address tweaks stay current) then strips ONLY those `hostdev`s; `off` re-inserts them from the saved fragment. It runs **shut-off only**, enforces a **device-diff safety invariant** (a whitespace-normalized fingerprint of every `<devices>` child except the GPU/audio `hostdev`s must be identical before vs after — it REFUSES to define if anything else changed), and `virt-xml-validate`s before `virsh define`.
- **One-release fallback:** if the GPU fragment is missing on a toggle off (e.g. upgraded from a pre-R44 install), it tries ONCE to recover the fragment from an old full backup, then **refuses** rather than invent a `hostdev` from vendor:device IDs — so you re-add the GPU in virt-manager once, or re-run `--install-live-attach` (VM shut off with the GPU in its XML) to recreate the fragment.
- `live_attach_toggle` recognizes each shut-off VM, resolves the target (`on`/`off` force, or `toggle` = flip the RECOGNIZED mode), applies it, and flips the global mode file ONLY if ≥1 shut-off VM was actually redefined — a running/refused/unknown run prints a mismatch instead of silently flipping the file. The libvirt hook also gained a recognition guard (if `mode=on` but the XML already has the GPU, no second hotplug), and the tray applet shows the RECOGNIZED mode and turns yellow ("XML not updated yet") when the mode file disagrees with the recognized mode.

**Boot-display pin — keep the GPU's guest PCI slot free (R44 follow-up).** In mode=on the GPU `hostdev` is stripped from the persistent XML, so the GPU's dedicated `pcie-root-port` (e.g. guest bus 0x06) is free at boot. The live-attach boot display (a virtio-gpu, added so Windows is not headless while the GPU is absent) is now **pinned to a different `pcie-root-port`** whose bus is NOT the GPU's and NOT the audio's reserved guest bus. Without this pin libvirt auto-placed the boot display on the freed GPU slot (bus 0x06), and the later `virsh attach-device --live` of the GPU at its fixed guest address (bus 0x06, via `VFIO_LIVE_ATTACH_KEEP_GUEST_ADDR=1`) **collided** → libvirt reassigned the hot-plugged GPU to another bus (e.g. 0x07) → Windows saw a **new** device instance instead of the one it installed the AMD driver against → Device Manager *"No drivers are installed for this device"* (Code 28), black screen. The pin leaves the GPU's slot free so the hot-plugged GPU lands at the same slot Windows knows → the AMD driver starts. Reserved buses are read from the per-VM device fragments (`profiles/<dom>/devices/{gpu,gpu-audio}.xml`) and passed to the patcher via `VFIO_LA_RESERVED_GUEST_BUSES`. **Important — a hot-plugged GPU has no UEFI GOP framebuffer** (OVMF only runs the option ROM for boot-present devices), so on a *fresh* Windows install with no AMD driver the card is still Code 28 regardless of slot: hotplug needs **one priming cold-attach boot** (`--install-dynamic-binding`, then install Adrenalin) so the driver binds to the GPU's guest slot; after that, hotplug reuses that exact slot. Cold-attach remains the reliable path.
...
**System-tray applet (`/usr/local/bin/vfio-hotplug-tray`).** A PySide6 `QSystemTrayIcon` app — KDE Plasma 6 uses the StatusNotifierItem protocol natively (Qt6), so the icon actually appears (XEmbed / `zenity --notification` would be invisible on Plasma 6). It reads the world-readable mode file (no root), refreshes its icon/tooltip every 2s, and the **Toggle hotplug** menu item runs a `zenity --question` confirmation → `pkexec vfio --live-attach-toggle` → `notify-send` with the result (normal urgency on success, critical on failure). A **Status…** menu item shows the full `--live-attach-status` output in a zenity info dialog. A per-user **autostart** desktop file (`~/.config/autostart/vfio-hotplug-tray.desktop`, installed for `SUDO_USER`) starts it on next Plasma login, and the installer best-effort launches one instance immediately. The icon itself is clickable — **left-click** toggles the hotplug (same zenity confirmation → pkexec → notify-send flow, so a stray click never changes anything silently), **middle-click** opens the Status dialog, and **right-click** pops the context menu (Toggle / Status / Quit).

**ReBAR / AMD-display banner (R45).** The tray applet also watches for the AMD ReBAR black-screen trap: it reads `/proc/cmdline` (world-readable, no root) + the guest GPU vendor from the conf, and when the guest GPU is AMD **and** `amdgpu.rebar=0` is missing from the kernel cmdline it shows a `⚠ ReBAR is ON — AMD display will go black` menu item — clicking it pops a zenity info dialog naming the fix (`amdgpu.rebar=0` on the kernel cmdline, or `sudo vfio --install-dynamic-binding --amd-rebar`; `amdgpu.resizable_bar` is NOT a real param and is silently ignored) — and appends a warning line to the tooltip. When the param is present the banner is absent so the tray stays clean. `--live-attach-status` now also emits `rebar_ok=0|1` (text + JSON) so non-GUI tooling can detect the at-risk state.
...
**Toggle OFF always restores the GPU to virt-manager (R43b).** Turning hotplug OFF redefines each shut-off guest-GPU VM from its saved with-GPU XML variant, so the PCIe GPU reappears in virt-manager and the VM boots normally under standard dynamic binding. If that with-GPU variant is missing or stale, `_la_ensure_with_gpu_variant` rebuilds it from the legacy backup / current XML / saved device XMLs — the operator never has to re-add the GPU by hand. (R43b fixed a latent bug where the GPU-presence check used a literal BDF grep that never matched libvirt's split `domain=/bus=/slot=/function=` attributes, so toggle OFF could skip the VM and leave the GPU removed; the check now reconstructs the BDF with awk, the same parser the install path uses.)
...
- **Hugepages (OPT-IN, the only host-RAM knob)**: when opted in, adds `<memoryBacking><hugepages>` + reserves the matching host `nr_hugepages` (prior value backed up, reverted by `--reset-ultimate-perf-vm-tuning`). Default OFF so a plain run never touches host RAM; all other perf knobs still apply without it. The reservation is hardened so a failed/short reservation never leaves a VM defined with a `<memoryBacking>` it can't satisfy:
  ...
  - **Boot-time auto re-reserve (reboot-persistent + regenerate)**: a systemd oneshot (`vfio-perf-hugepages-reserve.service`, installed automatically when hugepages is opted in) runs at every boot — *before* libvirt starts autostart VMs — and REGENERATES each per-VM owned count from the on-disk libvirt VM XML (`/etc/libvirt/qemu/<dom>.xml`), so a RAM grow/shrink between boots is honored automatically (no manual re-tune needed after a reboot + RAM change). It then reserves `nr_hugepages` = sum of all owned. A registry (`/var/lib/vfio-dynamic/perf-hugepages-dirs`) of perf backup dirs is maintained automatically; the service is removed by `--reset-ultimate-perf-vm-tuning` (when no hugepages VMs remain). `--reset` intentionally **leaves it in place** so a hugepages-backed VM still starts across reboots until you explicitly revert perf.
  - **Dynamic re-reserve at VM start**: the libvirt qemu hook's `prepare` phase now calls the same script's `--ensure-domain <dom>` mode (bounded timeout) **before** the GPU bind. It recomputes THIS VM's owned count from the live XML piped on stdin and grows `nr_hugepages` to cover the sum of all owned if needed (grow-only) — so a RAM change **without** a reboot is honored at the very moment qemu mmaps the backing store. No-op for non-hugepages VMs. On a fragmented-RAM shortfall it exits non-zero so libvirt aborts the VM start with a clear error instead of qemu's cryptic `unable to map backing store for guest RAM: Cannot allocate memory` — and **before** the GPU is ripped off the host, so a failed start leaves the host desktop untouched. (`VFIO_HOOK_HUGEPAGES_TIMEOUT` overrides the 10s ceiling.)
...
- **R36**: live-attach (hotplug GPU) opt-out now reverts an already-active setup. Previously, selecting "no" at the `--install-dynamic-binding` RX 9070 live-attach prompt only printed skip notes and left live-attach active (the VM XML still had the GPU hostdev stripped, the helper/VM-list/backups stayed on disk, `VFIO_DYNAMIC_LIVE_ATTACH` stayed `1`) — the user reported "select no for hotplug it still on the system does not switch." Now "no" calls `remove_live_attach` (restores the GPU hostdev to each VM's XML from the per-VM `live-attach-backup-<dom>.xml` backup, detaches the virtio-win ISO, flips the conf to `0`) so the VM switches back to normal dynamic binding — mirroring the vBIOS/stealth opt-out-revert pattern. Two new override flags `--live-attach` (force on) / `--no-live-attach` (force skip + revert) via a new `LIVE_ATTACH_OVERRIDE` var; the RX 9070 recommendation prompt now only runs when no override is given (so the revert works for every GPU family, not just RX 9070 / RDNA4). `remove_live_attach` is now best-effort (`return 0`) so the bare calls under `set -e` (the dynamic opt-out, `--install-early-binding`, `--reset`) no longer abort mid-cleanup when live-attach was never active.
