# Changelog
## Unreleased
- 2026-08-10 13:34 UTC: Streamlined the full-wizard dynamic-binding install path in `vfio.sh` so choosing dynamic binding now auto-adds the RX 9070 / RDNA4 reset-bug stability kernel params and skips the optional prompts, distro-aware (existing early-binding interactive flow kept intact):
  - when the user chooses dynamic binding in the full wizard, the three bootloader functions (`systemd_boot_add_kernel_params` openSUSE `/etc/kernel/cmdline` block, `grub_add_kernel_params` classic-GRUB block, and the generic systemd-boot current-entry block) now NON-interactively auto-add `vfio-pci.disable_idle_d3=1`, `pcie_port_pm=off`, and `pcie_aspm=off` when the guest GPU is AMD (gated on `CTX[guest_vendor]==1002`), matching the params the standalone `--install-dynamic-binding` path already ensures via `ensure_amd_reset_bug_params` plus `pcie_aspm=off`; the reset-bug pair honors `--no-amd-disable-idle-d3` / `--no-amd-pcie-port-pm-off` opt-outs,
  - the same dynamic path now SKIPS the optional prompts that early binding still asks (USB/xHCI power-management, boot framebuffer, the AMD 4-param stability workaround prompt, SELinux/AppArmor, ACS override, boot verbosity, boot target, and custom kernel params) so a dynamic install is one streamlined pass instead of ~7 yes/no questions,
  - `apply_configuration()`'s outer "Enable IOMMU kernel parameters now?" gateway prompt now auto-proceeds in dynamic mode (`if [[ DYNAMIC ]] || prompt_yn …`) since IOMMU is mandatory for passthrough in both modes, and prints a note that it is auto-proceeding; early binding still asks (default Y),
  - the persistence target is distro-aware: openSUSE writes `/etc/kernel/cmdline` + syncs BLS via `sdbootutil`, classic GRUB edits `/etc/default/grub` + runs `grub-mkconfig`, generic systemd-boot edits the selected entry in-place — the same auto-add + skip logic runs in whichever branch the detected bootloader takes,
  - the `vfio-pci.ids=` strip and the `rd.driver.pre=vfio-pci` dynamic-skip guards are unchanged (dynamic still strips both); the boot-metadata recovery + BLS sync in the openSUSE block stays outside the wrap so root/rootflags are preserved,
  - verified the openSUSE dynamic path produces exactly `amd_iommu=on iommu=pt vfio-pci.disable_idle_d3=1 pcie_port_pm=off pcie_aspm=off` (the user's current cmdline stability set), so a fresh dynamic install lands those params without any prompts,
  - `bash -n` + `shellcheck` clean on `vfio.sh`; full `regression/script.sh` suite = 17 PASS / 0 FAIL (custom-kernel-params + dynamic-binding regressions still pass because the `add_param_once "$new_cmdline" "vfio-pci.disable_idle_d3=1"` / `pcie_port_pm=off` strings remain in the early-binding `else` branch).
