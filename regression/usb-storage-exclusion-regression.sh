...
# Case 1: Interlock mode - missing storage at first, choose re-entry path, then include storage.
...
assert_contains_text "case1 danger warning shown" "DANGER: Some storage devices are NOT excluded from detach." "$case1_stdout_text"

# Case 2: Interlock mode - missing storage, decline re-entry, accept explicit risk phrase.
...
assert_eq "case2 confirm prompt matches expected safety warning" "Proceeding without keeping all storage devices host-bound is risky." "$last_confirm_prompt"
assert_eq "case2 confirm phrase matches expected safety phrase" "I ACCEPT STORAGE RISK" "$last_confirm_phrase"
assert_contains_text "case2 danger warning shown" "DANGER: Some storage devices are NOT excluded from detach." "$case2_stdout_text"

# Case 3: Interlock mode - missing storage, decline re-entry, reject risk phrase, then re-enter and include storage.
...
# Case 6: Default Bluetooth-only policy - missing storage does not force interlock confirmation.
...
assert_eq "case6 keeps selected non-storage exclusion in default policy" "bbbb:0002" "$case6_exclude_ids"
...
assert_contains_text "case6 info note shown for unselected storage" "Info: some storage devices are not selected." "$case6_stdout_text"
assert_contains_text "case6 policy note clarifies bluetooth-only behavior" "Current policy is Bluetooth-only (MATCH_MODE=auto with empty INCLUDE_IDS), so storage devices are not detach targets." "$case6_stdout_text"
assert_not_contains_text "case6 danger warning not shown in default policy" "DANGER: Some storage devices are NOT excluded from detach." "$case6_stdout_text"
