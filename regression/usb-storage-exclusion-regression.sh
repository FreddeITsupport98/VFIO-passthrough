...
assert_contains_text "case17 hard-block flow warns about VM passthrough impact" "Passthrough impact: while authorized=0 is active, matched USB devices cannot be attached to VMs." "$case17_stdout_text"
assert_contains_text "case17 hard-block flow explains why feature exists" "Why this exists: some unstable adapters keep reset/timeout looping even after detach-only mitigation." "$case17_stdout_text"
